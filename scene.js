...
scene.add(new THREE.HemisphereLight(0xffffff,0x71eeb8,2.2));const key=new THREE.DirectionalLight(0xffffff,3);key.position.set(4,6,5);scene.add(key);const rim=new THREE.PointLight(0x6e5cff,45,18);rim.position.set(-4,2,4);scene.add(rim);
const group=new THREE.Group();scene.add(group);
const mats=[new THREE.MeshStandardMaterial({color:0x20c997,roughness:.38,metalness:.25}),new THREE.MeshStandardMaterial({color:0x87f0cf,roughness:.5,metalness:.12}),new THREE.MeshStandardMaterial({color:0x6e5cff,roughness:.32,metalness:.35})];
